...
[.lang-python]
NOTE: Python has no method overloading, so the method with the per-execution properties gets its own name (`send_with_properties`); repeatable annotations such as `@MqttProperty` are stacked as decorators and a nullable argument is typed `str | None`.
...
[.lang-python]
For method arguments, if the value is not supplied to the annotation, the argument name will be used as the property name. For example, `userId: Annotated[str, MqttProperty]` would result in the property `userId` being set on the properties object before publishing.
