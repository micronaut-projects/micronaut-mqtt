...
[.lang-python]
NOTE: Python has no method overloading, so each subscriber method needs its own name (`receive_from_topic`); repeatable annotations such as `@Topic` are stacked as decorators.
