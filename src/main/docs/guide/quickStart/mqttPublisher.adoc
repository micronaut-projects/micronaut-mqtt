...
[.lang-python]
NOTE: In Python the publisher is an abstract class (`ABC`) whose abstract methods have `...` bodies; Micronaut generates the implementation.
...
[.lang-python]
NOTE: Because the `send` method returns `None` this means the method will publish the message and block until the broker acknowledges the message.
