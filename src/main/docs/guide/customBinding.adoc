...
[.lang-python]
NOTE: In Python a custom annotation is a function returning a decorator, meta-annotated like the Java annotation type.
...
[.lang-python]
NOTE: In Python the binder implements `AnnotatedMqttBinder` without type arguments; `getAnnotationType` returns the generated annotation type (a `java.lang.Class`), obtained with `java.type(...)` because the Python annotation function itself is not a Java class.
...
[.lang-python]
NOTE: In Python the binder implements `TypedMqttBinder` without type arguments; `getArgumentType` returns `Argument.of(ProductInfo)` with the imported Python class.
